...
import {
  ...
  V3_SUBJECT_DEFINITIONS,
  type V3SubjectDefinition,
} from "./v3PromptPack.js";
import {
  buildV4PromptOverlay,
  buildV4SubjectPromptSection,
...
type SubjectDefinition = V3SubjectDefinition | V4SubjectDefinition;

function buildSubjectPrompt(subject: SubjectDefinition, articles: GCAMArticle[]): string {
  const isV3 = subject.name.startsWith("v3_");
  const subjectPrompt = isV3 ? buildV3SubjectPromptSection(subject as V3SubjectDefinition) : buildV4SubjectPromptSection(subject as V4SubjectDefinition);
  const articlePayload = buildArticlePayload(articles);
  const subjectHardRules =
    subject.name === "v3_02_political_leadership" ||
    subject.name === "v3_03_national_security" ||
    subject.name === "v4_02_state_leadership" ||
    subject.name === "v4_03_national_security"
      ? [
...
المواد والذرات المرتبطة بهذا الموضوع:
${articlePayload}

قواعد تشغيل هذا الموضوع:
1. أنت تفحص موضوعاً واحداً فقط: "${subject.titleAr}".
2. لا تُرجع findings لموضوع آخر حتى لو ظهر في النص. اتركه لمساره الخاص.
3. استخدم ذاكرة القصة المرفقة لفهم المتحدث، المستهدف، النبرة، موقف السرد، وما إذا كان السلوك مُداناً أو مُطبّعاً.
4. لا تستخدم ذاكرة القصة كدليل حرفي. evidence_snippet و location يجب أن يأتيا من مقطع النص الحالي فقط.
...
function getSubjectByName(name: string): SubjectDefinition | undefined {
  return getActiveSubjectDefinitions().find((item) => item.name === name);
}

function isPoliticalOrSecurityPass(passName: string): boolean {
  return (
    passName === "v3_02_political_leadership" ||
    passName === "v3_03_national_security" ||
    passName === "v4_02_state_leadership" ||
    passName === "v4_03_national_security"
  );
}

function isChildOrWomenSensitivePass(passName: string): boolean {
  return (
    passName === "v3_12_women_abuse" ||
    passName === "v3_08_child_disability_harm" ||
    passName === "v4_08_child_disability_harm"
  );
}
...
const LEGACY_DETECTION_PASSES: PassDefinition[] = [
  {
    name: "glossary",
...
function getPerPassMaxFindings(passName: string): number {
  if (isPoliticalOrSecurityPass(passName)) return 14;
  if (isChildOrWomenSensitivePass(passName)) return 16;
  return 20;
}
...
function applyEarlyPassFilters(passName: string, findings: JudgeFinding[], chunkText?: string): { filtered: JudgeFinding[]; dropped: number } {
  ...
  for (const finding of findings) {
    ...
    if (isPoliticalOrSecurityPass(passName)) {
...
async function runSinglePass(
  ...
  try {
    ...
    logger.info("Judge Call Diagnostics", {
      ...
      maxTokens: config.AI_PROVIDER === "gemini" ? 32768 : 8192,
      thinkingBudget: config.AI_PROVIDER === "gemini" ? 16384 : undefined,
      finishReason: judgeCall.finish_reason,
      repairAttempts,
    ...
    const tagged = findingsWithIdentity.map((f) => ({
      ...
      article_id: (!f.article_id || f.article_id === 0) && isV5Reviewer && articleIds.length === 1 ? articleIds[0] : f.article_id,
